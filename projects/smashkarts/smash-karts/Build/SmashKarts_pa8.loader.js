function createUnityInstance(e, t, r) {
    ...
    function o(e) {
        ...
        r += "\n" + n.trim(),
        r && c.stackTraceRegExp && c.stackTraceRegExp.test(r)) {
    ...
    function s(e, t, r) {
        if (e.indexOf("fullscreen error") == -1) {
            if (c.startupErrorHandler)
                return void c.startupErrorHandler(e, t, r);
    ...
    c.QuitCleanup = function() {
        for (var e = 0; e < c.deinitializers.length; e++)
            c.deinitializers[e]();
        c.deinitializers = [],
        "function" == typeof c.onQuit && c.onQuit()
    }
    ;
    var p = ""
      , g = "";
    document.addEventListener("webkitfullscreenchange", function(t) {
        ...
        r === e ? e.style.width && (p = e.style.width,
        g = e.style.height,
        e.style.width = "100%",
        e.style.height = "100%") : p && (e.style.width = p,
        e.style.height = g,
        p = "",
        g = "")
    });
            ...
            fetch(t, r).then(function(t) {
                function r() {
                    return "undefined" == typeof a ? t.arrayBuffer().then(function(e) {
                        ...
                        return e.done ? o() : (l + e.value.length <= d.length ? (d.set(e.value, l),
                        c = l + e.value.length) : u.push(e.value),
                        l += e.value.length,
                        n({
                            type: "progress",
                            total: Math.max(i, l),
                ...
                var a = "undefined" != typeof t.body ? t.body.getReader() : void 0
                  ...
                  , l = 0
                  , c = 0;
                return s || console.warn("[UnityCache] Response is served without Content-Length header. Please reconfigure server to include valid Content-Length for better download performance."),
                r().then(function(e) {
                    return n({
                        type: "load",
                        total: e.length,
        ...
        function s(s, l) {
            function c(t, r) {
                return u(t, r).then(function(t) {
                    return !g.enabled || g.revalidated ? t : 304 === t.status ? (g.result.revalidated = g.result.accessed,
                    g.revalidated = !0,
                    h.storeRequest(g.result).then(function() {
                    ...
                    new n(g.result.response)) : (200 == t.status ? (g.result = o(t.url, g.company, g.product, g.accessed, t),
                    g.revalidated = !0,
                    h.storeRequest(g.result).then(function() {
            ...
            return l && (g.control = l.control,
            g.company = l.company,
            g.product = l.product),
            g.result = o(p, g.company, g.product, Date.now()),
            g.revalidated = !1,
            g.enabled ? h.loadRequest(g.result.url).then(function(t) {
                if (!t || t.version !== d.version)
                    return c(s, l);
                g.result = t,
                g.result.accessed = Date.now();
                var o = new n(g.result.response);
                if ("immutable" == g.control)
                    return g.revalidated = !0,
                    h.storeRequest(g.result),
                    e("'" + g.result.url + "' served from the indexedDB cache without revalidation"),
                    f(o),
                    o;
                if (r(g.result.url) && (o.headers.get("Last-Modified") || o.headers.get("ETag")))
                    return fetch(g.result.url, {
                        method: "HEAD"
                    }).then(function(t) {
                        return g.revalidated = ["Last-Modified", "ETag"].every(function(e) {
                            return !o.headers.get(e) || o.headers.get(e) == t.headers.get(e)
                        }),
                        g.revalidated ? (g.result.revalidated = g.result.accessed,
                        h.storeRequest(g.result),
                        e("'" + g.result.url + "' successfully revalidated and served from the indexedDB cache"),
                        f(o),
                        o) : c(s, l)
                ...
                return e("Failed to load '" + g.result.url + "' from indexedDB cache due to the error: " + t),
                u(s, l)
            }) : u(s, l)
        }
        var i = c.UnityCache
    ...
    new Promise(function(e, t) {
        ...
        l()) : t("Your browser does not support WebAssembly.") : t("Your browser does not support WebGL.")
    }
    )
}
